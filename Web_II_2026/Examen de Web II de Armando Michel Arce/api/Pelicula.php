    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            if (empty($nombre)) {
                ...
                echo json_encode(["error" => "Falta el nombre de la pelicula (obligatorio)"]);
                exit();
            }

            $stmt = $conn->prepare("INSERT INTO pets (id, nombre) VALUES (?, ?)");
            $stmt->bind_param("ss", $id, $nombre);

            if ($stmt->execute()) {
                http_response_code(201);
                echo json_encode(["message" => "pelicula creada", "id" => $id, "nombre" => $nombre]);
            } else {
                http_response_code(500);
        ...
        case 'PUT':
            ...
            if ($stmt->execute()) {
                http_response_code(200);
                echo json_encode(["message" => "pelicula actualizada", "id" => $id, "nombre" => $nombre, "edad" => $edad]);
            } else {
                http_response_code(500);
