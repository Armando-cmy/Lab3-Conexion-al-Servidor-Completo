<?php
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            $stmt = $conn->prepare("INSERT INTO boleto (id, nombre, precio, descripcion) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssds", $id, $nombre, $precio, $descripcion);

            if ($stmt->execute()) {
                http_response_code(201);
                echo json_encode(["message" => "boleto creado", "id" => $id, "nombre" => $nombre, "precio" => $precio, "descripcion" => $descripcion]);
            } else {
                http_response_code(500);
        ...
        case 'PUT':
            ...
            $stmt = $conn->prepare("UPDATE boleto SET nombre = ? Ticket = ? WHERE id = ?");
            $stmt->bind_param("sdss", $nombre, $precio, $descripcion, $id);
            if ($stmt->execute()) {
                http_response_code(200);
                echo json_encode(["message" => "boleto actualizado", "id" => $id, "nombre" => $nombre, "precio" => $precio, "descripcion" => $descripcion]);
            } else {
                http_response_code(500);
